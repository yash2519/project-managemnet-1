...
import { dailyTimelineBuilder } from "../engine/DailyTimelineBuilder";
...
export const getDailyTimeline = async (
  ...
  try {
    ...
    if (!isOwner && !isAdmin && !hasAssignedTask) {
      res
        .status(403)
        .json({ message: "Forbidden: insufficient permissions to view daily timeline" });
      return;
    }

    const result = await dailyTimelineBuilder.buildDailyTimeline(Number(projectId), date);
    ...
    console.error("Daily Timeline Controller Error:", error);
    res
      .status(500)
      .json({ message: `Error retrieving daily timeline: ${error.message}` });
  }
};
